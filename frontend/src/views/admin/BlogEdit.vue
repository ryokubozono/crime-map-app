  ...
  <div>
    ...
    <v-form>
      ...
      <v-row>
        <v-col>
          <v-btn
            v-on:click="$router.push({name: 'blog_show', params: {blog_id: blog_id}})"
          >
            back
          </v-btn>
        </v-col>
  ...
  methods: {
    ...
    async get_blog() {
      ...
      .then(res => {
        ...
        this.blog.is_visible = res.data.is_visible,
    ...
    async update_blog() {
      await axios.patch(`${process.env.VUE_APP_FAST_API_URL}/api/blogs/update_blog/${this.blog_id}/`, {
